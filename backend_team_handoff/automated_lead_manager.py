...
class AutomatedLeadManager:
    ...
    def __init__(self, crm_api_url="https://blyqk7itsc.execute-api.us-east-1.amazonaws.com/prod", config_file=None):
    ...
    def get_current_lead_metrics(self) -> Dict:
        """Get lead metrics from LAMBDA BACKEND (not local database)"""
        try:
            # Check Lambda backend health endpoint for current lead stats
            health_url = f"{self.crm_api_url}/health"
            response = requests.get(health_url, timeout=10)
            
            if response.status_code != 200:
                logger.error(f"Failed to get Lambda backend health: {response.status_code}")
                return {}
            
            health_data = response.json()
            db_stats = health_data.get('database_stats', {})
            
            # Extract Lambda backend lead counts
            total_leads = db_stats.get('total_leads', 0)
            available_leads = db_stats.get('active_leads', 0)
            
            # Since Lambda doesn't track priority counts, estimate based on typical distribution
            # A+ = 20%, A = 50%, B = 30%
            priority_counts = {
                'A+': int(available_leads * 0.2),
                'A': int(available_leads * 0.5), 
                'B': int(available_leads * 0.3),
                'C': 0
            }
            
            # Default conversion rates (Lambda doesn't track these yet)
            conversion_rates = {'A+': 0.15, 'A': 0.10, 'B': 0.05}
            
            # Active agents from Lambda
            active_agents = db_stats.get('active_agents', 1)
            
            logger.info(f"📊 Lambda Backend Stats: {total_leads} total leads, {available_leads} available")
            ...
            logger.error(f"Error getting Lambda backend metrics: {e}")
            # Fallback to minimal values that will trigger upload
            return {
                'total_leads': 0,
                'available_leads': 0,
                'priority_counts': {'A+': 0, 'A': 0, 'B': 0, 'C': 0},
                'conversion_rates': {'A+': 0.15, 'A': 0.10, 'B': 0.05},
                'active_agents': 1,
                'timestamp': datetime.utcnow(),
    ...
    def execute_automated_upload(self, selected_leads: List[Dict]) -> bool:
        ...
        try:
            ...
            if 'uploaded_npis' not in self.upload_history:
                self.upload_history['uploaded_npis'] = []
            self.upload_history['uploaded_npis'].extend(uploaded_npis)
