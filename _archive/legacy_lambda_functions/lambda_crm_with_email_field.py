...
# Enhanced in-memory leads storage with EMAIL, PTAN and EIN/TIN fields
LEADS = {
    1: {
        "id": 1,
        ...
        "email": "",  # Practice email address (NEW FIELD)
    ...
    2: {
        ...
        "email": "info@desertortho.com",  # Practice email address (NEW FIELD WITH EXAMPLE)
...
def prepare_external_api_payload(lead_data, user_info):
    """Prepare payload for external createUserExternal API with REAL EMAIL addresses"""
    try:
        # Use the actual email from the lead, fallback to generated if empty
        practice_email = lead_data.get('email', '').strip()
        if not practice_email:
            # Fallback to generated email if no real email provided
            practice_email = f"info@{lead_data['practice_name'].lower().replace(' ', '').replace('-', '')}.com"
        
        # Use practice email as primary contact email (most common scenario)
        primary_contact_email = practice_email
        
        # Map our CRM data to their API format
        payload = {
            "email": practice_email,  # Use REAL email from lead
            ...
            "primaryContactEmail": primary_contact_email,  # Use REAL email
        ...
        print(f"📧 Using Email: {practice_email}")
...
def lambda_handler(event, context):
    """AWS Lambda handler with complete CRM functionality + Send Docs + Email Field"""
    ...
    try:
        ...
        if path == '/health':
            ...
            return {
                ...
                'body': json.dumps({
                    ...
                    'version': '4.1.0',
                    'service': 'Cura Genesis CRM - Complete Production System with Send Docs + Email',
                    'database': 'connected',
                    'database_stats': {
        ...
        if path.startswith('/api/v1/leads/') and method == 'PUT':
            ...
            # Update lead fields (INCLUDING NEW EMAIL, PTAN/EIN FIELDS)
        ...
        # 🚀 ENHANCED: SEND DOCS ENDPOINT (NOW WITH REAL EMAIL SUPPORT)
        if path.startswith('/api/v1/leads/') and path.endswith('/send-docs') and method == 'POST':
            ...
            # Enhanced validation: Check if email is provided
            practice_email = lead.get('email', '').strip()
            if not practice_email:
                return {
                    'statusCode': 400,
                    'headers': response_headers,
                    'body': json.dumps({'detail': 'Practice email address is required before sending documents. Please add the email address and try again.'})
                }
            ...
            if result['success']:
                ...
                ACTIVITIES.append({
                    ...
                    'description': f"Documents prepared for external system. Email: {result.get('email_used', 'N/A')}, User ID: {result.get('user_id', 'N/A')}",
                    'outcome': 'success',
                    'created_at': datetime.utcnow().isoformat()
                })
                NEXT_ACTIVITY_ID += 1
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'available_endpoints': [
                    ...
                    'GET /api/v1/leads',
                    'PUT /api/v1/leads/{id}',
                    'POST /api/v1/leads/{id}/send-docs',  # ENHANCED WITH EMAIL VALIDATION
                    'GET /health'
                ]
            })
        }
