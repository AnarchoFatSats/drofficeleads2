...
def send_docs_to_external_api(lead_data, user_info):
    ...
    try:
        ...
        payload = {
            "email": f"{lead_data['practice_name'].lower().replace(' ', '.')}@{lead_data['practice_name'].lower().replace(' ', '')}.com",  # Generate email from practice name
            ...
            "facilityAddress": {
                ...
                "fax": lead_data.get('fax', lead_data.get('practice_phone', ''))  # Use phone as fallback for fax
            },
            ...
            "additionalPhysicians": []  # Empty for now, could be populated if we have multiple providers
        }
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if path.startswith('/api/v1/leads/') and path.endswith('/send-docs') and method == 'POST':
            ...
            result = send_docs_to_external_api(lead, current_user)
            ...
            if result['success']:
                ...
                ACTIVITIES.append({
                    ...
                    'description': f"Documents sent to external system. User ID: {result.get('user_id', 'N/A')}",
                    'outcome': 'success',
                    'created_at': datetime.utcnow().isoformat()
                })
                NEXT_ACTIVITY_ID += 1
                ...
                ACTIVITIES.append({
                    ...
                    'description': f"Failed to send documents: {result['message']}",
                    'outcome': 'failed',
                    'created_at': datetime.utcnow().isoformat()
                })
                NEXT_ACTIVITY_ID += 1
        ...
        if path == '/api/v1/analytics/performance' and method == 'GET':
            ...
            docs_sent = len([l for l in LEADS.values() if l.get('docs_sent', False)])
            ...
            for user in USERS.values():
                if user['role'] == 'agent':
                    ...
                    agent_docs_sent = len([l for l in agent_leads if l.get('docs_sent', False)])
