...
class APISmartLeadInjection:
    ...
    def upload_lead_batch(self, leads_batch):
        """Upload a batch of leads via API"""
        try:
            response = requests.post(
                f"{self.base_url}/api/v1/leads/bulk",
                json=leads_batch,
                headers=self.headers
            )
            ...
            if response.status_code == 200:
                ...
                return result['summary']['created'], result['summary']['duplicates'], result['summary']['failed']
            else:
                print(f"❌ API error {response.status_code}: {response.text}")
                return 0, 0, len(leads_batch)
    ...
    def execute_phase_upload(self, phase_key, source_leads):
        ...
        for i in range(0, len(api_leads), self.batch_size):
            ...
            success, duplicates, errors = self.upload_lead_batch(batch)
            total_success += success
            total_duplicates += duplicates
            total_errors += errors
            
            print(f"   ✅ Batch result: {success} created, {duplicates} duplicates, {errors} errors")
